...
public:
    QAscTabWidget(QWidget *parent = 0) : QTabWidget(parent)
    {
        ...
        QObject::connect(this, SIGNAL( sendUpdateIcons()), this, SLOT( slotUpdateIcons() ), Qt::QueuedConnection );
        ...
        connect(this, SIGNAL(tabBarClicked(int)), this, SLOT(currentChangedSlot(int)));
    }
    ...
    void addEditor(QString strName, AscEditorType etType = etDocument, std::wstring strUrl = L"")
    {
        ...
        // это обязательно!!! а то размеры проставляются только при переходе на таб
        pView->setGeometry(0, 0, this->width(), this->height() - this->tabBar()->height());
    ...
    void addTabUrl(std::wstring sUrl, QString sName)
    {
        if (!m_pManager)
            return;
        ...
        this->addTab(pView, sName);
        this->resizeEvent(NULL);

        pView->GetCefView()->load(sUrl);
    ...
    void addLocalFile(std::wstring strFilePath)
    {
        if (!m_pManager)
            return;

        int nFormat = CCefViewEditor::GetFileFormat(strFilePath);
        if (nFormat == 0)
            return;
        ...
        std::wstring sName = NSCommon::GetFileName(strFilePath);

        // это обязательно!!! а то размеры проставляются только при переходе на таб
        pView->setGeometry(0, 0, this->width(), this->height() - this->tabBar()->height());

        this->addTab(pView, QString::fromStdWString(sName));
        this->resizeEvent(NULL);

        ((CCefViewEditor*)pView->GetCefView())->OpenLocalFile(strFilePath, nFormat);

        AddLastTabCloseButton(pView);
    }
    void addLocalFileCreate(int nType)
    {
        if (!m_pManager)
            return;
        ...
        std::wstring sName = L"new.docx";
        if (etSpreadsheet == nType)
            sName = L"new.xlsx";
        else if (etPresentation == nType)
            sName = L"new.pptx";
        ...
        ((CCefViewEditor*)pView->GetCefView())->CreateLocalFile(nType, sName);

        AddLastTabCloseButton(pView);
    }
    void addLocalFileRecover(NSEditorApi::CAscLocalOpenFileRecent_Recover* pData)
    {
        if (!m_pManager)
            return;
        ...
        ((CCefViewEditor*)pView->GetCefView())->OpenRecoverFile(pData->get_Id());

        AddLastTabCloseButton(pView);
    }
    void addLocalFileRecent(NSEditorApi::CAscLocalOpenFileRecent_Recover* pData)
    {
        if (!m_pManager)
            return;
        ...
        ((CCefViewEditor*)pView->GetCefView())->OpenRecentFile(pData->get_Id());

        AddLastTabCloseButton(pView);
    }

    void AddLastTabCloseButton(QCefView* pView)
    {
        QPushButtonWrapper *close = new QPushButtonWrapper(pView);
        close->m_pView = pView;
...
public slots:
    void clickedSlotClose()
    {
        ...
        bool bIsModifyed = pViewCef->GetCefView()->GetModified();
        if (!bIsModifyed)
        {
            // закрываем таб
        ...
        else
        {
            ...
            reply = QMessageBox::question(NULL, "AscDesktopEditor", "Want to save you changes? ", QMessageBox::Yes | QMessageBox::No);
